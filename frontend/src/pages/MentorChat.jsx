    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: 'var(--bg-light)' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
        {chats.length === 0 ? (
          ...
          chats.map(c => (
            <div key={c.id} style={{ alignSelf: c.message_type === 'query' ? 'flex-end' : 'flex-start', maxWidth: '85%' }}>
              <div style={{
                background: c.message_type === 'query' ? '#2563eb' : 'white',
                ...
                {c.message_type === 'response' && inputType === 'text' && (
                  ...
                  >
                    <Volume2 size={14} /> {listenLabel[selectedLanguage]}
                  </button>
                )}
              </div>
            </div>
        ...
        {sending && (
          <div style={{ alignSelf: 'flex-start', maxWidth: '85%' }}>
            <div style={{ background: '#f3f4f6', padding: '12px 16px', borderRadius: 'var(--radius-lg)', border: '1px solid var(--gray-200)', display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span style={{ color: 'var(--gray-600)', fontSize: 'var(--font-sm)' }}>{thinkingMessages[selectedLanguage](selectedMentor.name)}</span>
            </div>
          </div>
        )}
      ...
      {/* Input Area */}
      <div style={{ background: 'white', padding: '16px', borderTop: '2px solid #2563eb', display: 'flex', gap: '10px', alignItems: 'flex-end', flexWrap: 'wrap', boxShadow: '0 -4px 12px rgba(0,0,0,0.1)' }}>
        {isListening && (
        ...
        <input
          ...
          placeholder={isListening ? (selectedLanguage === 'ta' ? 'கேளுகிறேன்...' : selectedLanguage === 'hi' ? 'सुन रहें हैं...' : 'Listening...') : (inputType === 'voice' ? (selectedLanguage === 'ta' ? 'பேச உங்கள் கேள்விக்கு உரை வாக்கு பதிவு செய்ய...' : selectedLanguage === 'hi' ? 'अपना प्रश्न बोलने के लिए माइक बटन का उपयोग करें...' : 'Use voice button to speak...') : (selectedLanguage === 'ta' ? 'உங்கள் கேள்வி கேளுங்கள்...' : selectedLanguage === 'hi' ? 'अपना प्रश्न पूछें...' : 'Ask your question...'))}
          value={message}
        ...
        {isSpeaking && (
          ...
          >
            <VolumeX size={22} /> {selectedLanguage === 'ta' ? 'தடை' : selectedLanguage === 'hi' ? 'रद्द करना' : 'Cancel'}
          </button>
        )}
      </div>
    </div>
